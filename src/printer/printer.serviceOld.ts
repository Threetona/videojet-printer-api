...
export class PrinterServiceOld {
    private readonly logger = new Logger(PrinterServiceOld.name);

    private createProtocolPacket(
        type: PacketType,
        fieldName: string,
        fieldData?: string, // data2 is optional and used for "Update Message Text"
    ): string {
        ...
        switch (type) {
            case PacketType.SetMessageParameters:
                packet = `${STX}${fieldName}${ETX}`;
                break;
            case PacketType.MessageSelect:
                packet = `${STX}${type}${fieldName}${ETX}`;
                break;
            ...
            case PacketType.UpdateMessageText:
                ...
                packet = `${STX}${type}${fieldName}${LF}${fieldData || ''}${ETX}`;
    ...
    async sendMultyCommandsToPrinter(
        commands: CommandDto[],
    ): Promise<string[]> {
    ...
    async sendSingleCommandsToPrinter(
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                const packet = this.createProtocolPacket(
                    ...
                    commandDto.fieldName,
                    commandDto.fieldData,
                );
                const checksum = this.calculateChecksum(packet);
